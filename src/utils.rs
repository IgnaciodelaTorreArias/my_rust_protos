...
pub(crate) fn get_call_message<T>(ptr: *const u8, len: usize) -> Result<T, CMR> where T: Default+prost::Message{
    if ptr.is_null(){
        Ok(T::default())
    }
    else {
        ...
        match T::decode(input) {
            Ok(msg) => Ok(msg),
            Err(e) => Err(CMR { details: e.to_string() })
        }
    }
}

pub(crate) fn set_call_result<T: prost::Message>(res: &T, out_ptr: *mut *mut u8, out_len: *mut usize) -> bool {
